...
void PrintHelp(int details) {
  if (!details) {
    ...
    std::cout
        ...
        << "bandwidth defined in board_spec.xml file in the OpenCL/oneAPI shim "
        << "provided the <OFS_OCL_SHIM_ROOT_HW> environment variable is set to "
        << "point to <path-to-OpenCL/oneAPI-shim/hardware/<board_variant>/> "
        << "directory.\n"
