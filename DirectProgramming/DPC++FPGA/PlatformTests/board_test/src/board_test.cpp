...
// The complete test comprises of 6 tests listed below
// User can choose to run all or select a specific test using options 0-6
// Usage: ./board_test.fpga to run all tests
// ./board_test.fpga -test=<test-number> to run a specific test
// ./board_test.fpga -help to print details about these tests
// Tests:
// 0 = Default (Run all tests)
// 1 = Host Speed Test
...
// Device Selection
// Select either:
//  - the FPGA emulator device (CPU emulation of the FPGA) using FPGA_EMULATOR macro
//  - the FPGA device (a real FPGA)
  ...
  try {
    ...
    if (test_to_run == 0 || test_to_run == 1) {
      ...
      if (ret_hostrw != 0) {
        std::cout << "Error: Host read write test failed with aligned dev "
                  << "address (dev_offset = "
                  << dev_offset << ")\n";
        std::cout << "\nBOARD TEST FAILED\n";
        return 1;
...
// access
      ...
      if (ret_hostrw != 0) {
        std::cout << "Error: Host read write test failed with unaligned dev "
           	  	  << "address (dev_offset = "
                  << dev_offset << ")\n";
        std::cout << "\nBOARD TEST FAILED\n";
        return 1;
    ...
    if (test_to_run == 0 || test_to_run == 3) {
      ...
      if (ret_kernel_launch_test == 0) {
        ...
        std::cout << "Error: Kernel Launch Test Failed\n";
        std::cout << "\nBOARD TEST FAILED\n";
        return 1;
      }
    ...
    if (test_to_run == 0 || test_to_run == 5) {
      ...
      if (ret_kernel_mem_rw != 0) {
        std::cout << "Error: kernel-memory read write test failed. \n";
        std::cout << "\nBOARD TEST FAILED\n";
        return 1;
      }
