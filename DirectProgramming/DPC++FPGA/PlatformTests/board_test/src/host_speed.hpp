...
struct Speed WriteSpeed(queue &q, buffer<char, 1> &device_buffer,
  ...
  for (size_t i = 0; i < num_xfers; i++) {
    ...
    evt[i] = q.submit([&](handler &h) {
      ...
      accessor<char, 1, access::mode::write> mem(device_buffer, h, buf_range,
                                                 buf_offset);
  ...
  speed_wr.slowest = 10000000.0f;

  for (size_t i = 0; i < num_xfers; i++) {
    float time_ns = SyclGetQSubExecTimeNs(evt[i]);
    float speed_MBps =
        ((float)block_bytes / kMB) / ((float)time_ns / 1000000000);
  ...
  speed_wr.average =
      ((float)total_bytes / kMB) / ((float)speed_wr.average / 1000000000);
  speed_wr.total =
      ((float)total_bytes / kMB) /
      ((float)SyclGetTotalTimeNs(evt[0], evt[num_xfers - 1]) / 1000000000);
...
struct Speed ReadSpeed(queue &q, buffer<char, 1> &device_buffer,
  ...
  for (size_t i = 0; i < num_xfers; i++) {
    ...
    evt[i] = q.submit([&](handler &h) {
      ...
      auto buf_offset = (i * block_bytes) / sizeof(char);
      // Accessor to access range of device buffer at buf_offset
      accessor<char, 1, access::mode::read> mem(device_buffer, h, buf_range,
                                                buf_offset);
  ...
  speed_rd.average =
      ((float)total_bytes / kMB) / ((float)speed_rd.average / 1000000000);
  speed_rd.total =
      ((float)total_bytes / kMB) /
      ((float)SyclGetTotalTimeNs(evt[0], evt[num_xfers - 1]) / 1000000000);
  ...
  for (auto j = 0; j < maxchars; j++) {
    if (hostbuf_rd[j] != hostbuf_wr[j]) {
      if (prints++ < 512) {  // only print 512 errors
        std::cout << "Error! Mismatch at element " << j << ":" << std::setw(8)
